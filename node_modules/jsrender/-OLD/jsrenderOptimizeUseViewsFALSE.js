...
this.jsviews || this.jQuery && jQuery.views || (function(global, undefined) {
	...
	function renderContent(data, context, path, key, parentView, onRender) {
		...
		var i, l, dataItem, newView, itemWrap, itemsWrap, itemResult, parentContext, tmpl, onRender, props, swapContent, isLayout,
			self = this,
			result = "",
			useViews = !context || context.useViews !== FALSE;
		...
		if (self.isTag) {
			...
			//
			useViews = parentView = parentView || self.view;
			path = path || self.path;
			key = key || self.key;
			props = self.props;
		} else {
	...
	//	parentView = parentView || { _hlp: getHelper, ctx: jsv.helpers }; // jsv.topView;
		parentContext = parentView ? parentView.ctx : jsv.helpers;
		if (tmpl) {
			if (data === parentView) {
			...
			onRender = useViews && (onRender || parentView && parentView._onRender);
			...
			if (tmpl) {
				if ($.isArray(data) && !isLayout) {
					...
					newView = useViews && (swapContent ? parentView : (key !== undefined && parentView) || View(context, path, parentView, data, tmpl, key, onRender, TRUE));
					for (i = 0, l = data.length; i < l; i++) {
						// Create a view for each data item.
						dataItem = data[i];
						itemResult = tmpl.fn(dataItem, useViews && View(context, path, newView, dataItem, tmpl, (key || 0) + i, onRender), jsv);
